...
int Request::parse_request(const char *read_buffer, int read_size) {
  ...
  while (req_buf->good()) {
    ...
    if (parse_part == PARSE_REQ_LINE) {  // parse request line like  "GET /index.jsp HTTP/1.1"
      ...
      // printf("parse_request_line success which method:%s, url:%s,
      // http_version:%s\n", this->line.method.c_str(),
      // this->line.request_url.c_str(), this->line.http_version.c_str());
    ...
    if (parse_part == PARSE_REQ_HEAD && !line.empty()) {
      ...
      SocketUtils::split_str(line, ':',
                             parts);  // line like Cache-Control:max-age=0
      if (parts.size() < 2) {
        continue;
      }
      // strncasecmp()  TODO
